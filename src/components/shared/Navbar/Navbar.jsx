  ...
  return (
    <>
      <nav ref={navbarRef} className="navbar">
        <div className={`nav-icon ${isOpen ? 'open' : ''}`} onClick={handleClick}>
          <img src={isOpen ? './BurgerClose.svg' : './BurgerSquare.svg'} alt="" />
        </div>
        <div className={`nav-links ${isOpen ? 'active' : ''}`}>
          <ul className="links">
            {menuList.map((menu) => (
              <li className="nav-link" key={menu.text}>
                <a onClick={() => { handleClick(); scrollToSection(menu.text.toLowerCase()); }} href={menu.link} className={menu.text.toLowerCase() === activeSection.toLowerCase() ? 'active' : ''}>
                  {menu.text}
                </a>
              </li>
            ))}
